...
def check_left_biphasic_p(triplet, ecg_lead, zcs, delineation):
    ...
    biphasic_th_more_peak_zc = float(PParams['ALPHA_BIPHASE_AMPL_MORE']) * abs(right_peak_zc.right_mm.value)
    biphasic_th_less_peak_zc = float(PParams['ALPHA_BIPHASE_AMPL_LESS']) * abs(right_peak_zc.right_mm.value)
    ...
    if biphasic_th_more_peak_zc < abs(left_peak_zc.left_mm.value) < biphasic_th_less_peak_zc:
        ...
        if is_prev_zc_exist(zcs, left_peak_zc_id, float(PParams['ALPHA_PEAK_BEGIN_SHIFT']) * rate):
            ...
            biphasic_th_more_prev_zc = float(PParams['ALPHA_BIPHASE_AMPL_MORE']) * abs(prev_zc.right_mm.value)
            biphasic_th_less_prev_zc = float(PParams['ALPHA_BIPHASE_AMPL_LESS']) * abs(prev_zc.right_mm.value)
            ...
            if biphasic_th_more_prev_zc < abs(prev_zc.left_mm.value) < biphasic_th_less_prev_zc:
                ...
                if prev_zc.mm_amplitude > float(PParams['ALPHA_BIPHASE_AMPL']) * amplitude:
    ...
    if biphasic_th_more_peak_zc < abs(left_peak_zc.left_mm.value) < biphasic_th_less_peak_zc:
        ...
        if is_next_zc_exist(zcs, right_peak_zc_id, float(PParams['ALPHA_PEAK_BEGIN_SHIFT']) * sampling_rate):
            ...
            if biphasic_th_more_next_zc < abs(next_zc.left_mm.value) <= biphasic_th_less_next_zc:
                ...
                if next_zc.mm_amplitude > float(PParams['ALPHA_BIPHASE_AMPL']) * amplitude:
                    delineation.specification = WaveSpecification.biphasic
