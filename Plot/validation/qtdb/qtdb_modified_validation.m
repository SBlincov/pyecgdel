...
for record = start_record : start_record + num_records - 1
    ...
    for lead_id = 1:2
        ...
        qrs_onset = qrs_del(:, 1) + 1;
        qrs_peak = qrs_del(:, 2) + 1;
        qrs_offset = qrs_del(:, 3) + 1;
            
        for i = 2:size(qrs_del, 1)
            rr_mean(lead_id) = rr_mean(lead_id) + (qrs_peak(i) - qrs_peak(i-1));
        end
        
        rr_mean(lead_id) = rr_mean(lead_id) / (size(qrs_del, 1) - 1);
    ...
    if is_qrs == 1
        qrs_onset_doc = qrs_del_doc(:, 1);
        qrs_peak_doc = qrs_del_doc(:, 1);
        qrs_offset_doc = qrs_del_doc(:, 1);
      
        qrs_num_doc_dels = size(qrs_del_doc, 1);
        
        for lead_id = 1:2
            peak_id = 1;
            while (qrs_peaks{lead_id}(peak_id) < qrs_peak_doc(1)) && (abs(qrs_peaks{lead_id}(peak_id)- qrs_peak_doc(1)) / rate > tolerance)
                qrs_onsets{lead_id}(peak_id) = [];
                qrs_peaks{lead_id}(peak_id) = [];
                qrs_offsets{lead_id}(peak_id) = [];
            end
        end
        
        for qrs_doc_id = 2:qrs_num_doc_dels
            if (qrs_peak_doc(qrs_doc_id) - qrs_peak_doc(qrs_doc_id - 1)) > 1.5 * mean(rr_mean)
                for lead_id = 1:2
                    peak_id = 1;
                    while (qrs_peaks{lead_id}(peak_id) < qrs_peak_doc(qrs_doc_id - 1)) && (abs(qrs_peaks{lead_id}(peak_id)- qrs_peak_doc(qrs_doc_id - 1)) / rate > tolerance)
                        peak_id = peak_id + 1;
                    end
                    peak_id = peak_id + 1;
                    while ((peak_id <= size(qrs_peaks{lead_id}, 1) && qrs_peaks{lead_id}(peak_id) < qrs_peak_doc(qrs_doc_id)) && (abs(qrs_peaks{lead_id}(peak_id)- qrs_peak_doc(qrs_doc_id)) / rate > tolerance))
                        qrs_onsets{lead_id}(peak_id) = [];
                        qrs_peaks{lead_id}(peak_id) = [];
                        qrs_offsets{lead_id}(peak_id) = [];
                    end
        ...
        for lead_id = 1:2
            ...
            while (peak_id <= size(qrs_peaks{lead_id}, 1)) && (qrs_peaks{lead_id}(peak_id_curr) < qrs_peak_doc(end)) && (abs(qrs_peaks{lead_id}(peak_id_curr)- qrs_peak_doc(end)) / rate > tolerance)
                peak_id_curr = peak_id_curr + 1;
            end
            peak_id_curr = peak_id_curr + 1;
            while (size(qrs_peaks{lead_id}, 1) > peak_id_curr - 1) && (qrs_peaks{lead_id}(peak_id_curr) > qrs_peak_doc(end))
                qrs_onsets{lead_id}(peak_id_curr) = [];
                qrs_peaks{lead_id}(peak_id_curr) = [];
                qrs_offsets{lead_id}(peak_id_curr) = [];
            end
        end
        ...
        for lead_id = 1:2
            qrs_onset_diffs_curr = zeros(size(qrs_onsets{lead_id}, 1), 1);
            qrs_peak_diffs_curr = zeros(size(qrs_peaks{lead_id}, 1), 1);
            qrs_offset_diffs_curr = zeros(size(qrs_offsets{lead_id}, 1), 1);
            
            qrs_onset_diffs_ids_curr = zeros(size(qrs_onsets{lead_id}, 1), 1);
            qrs_peak_diffs_ids_curr = zeros(size(qrs_peaks{lead_id}, 1), 1);
            qrs_offset_diffs_ids_curr = zeros(size(qrs_offsets{lead_id}, 1), 1);
            
            for i = 1:min([size(qrs_peaks{1}, 1) size(qrs_peaks{2}, 1) size(qrs_peak_doc, 1)])
                
                curr_onset_diff = qrs_onsets{lead_id} - qrs_onset_doc(i);
                curr_peak_diff = qrs_peaks{lead_id} - qrs_peak_doc(i);
                curr_offset_diff = qrs_offsets{lead_id} - qrs_offset_doc(i);
        ...
        for i = 1:qrs_num_doc_dels
            
            qrs_onset_diff_id_curr = find(qrs_onset_diffs_ids{1} == i);
            if size(qrs_onset_diff_id_curr, 1) > 0
                ...
                qrs_onset_diff_id_lead_2 = find(qrs_onset_diffs_ids{2} == i);
        ...
        qrs_onset_FN = qrs_onset_FN + abs(qrs_num_doc_dels - qrs_onset_num_hits);
        qrs_peak_FN = qrs_peak_FN + abs(qrs_num_doc_dels - qrs_peak_num_hits);
        qrs_offset_FN = qrs_offset_FN + abs(qrs_num_doc_dels - qrs_offset_num_hits);
